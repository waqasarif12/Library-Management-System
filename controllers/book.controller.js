...
module.exports = {
    ...
    purchaseBook: async (req, res, next) => {
        ...
        if (alreadyPurchased && alreadyPurchased?.active) {
            // then show error
            req.flash("error","User has already purchased this book");
        ...
        if (!book) {
            // then show error
            req.flash('error', "Bokk Id does not exist")
            res.redirect("/book");
        }
        if (book && book?.quantity <= 0) {
            // then show error
            req.flash("succes", "Not sufficient book: ${book.book_name} to purchase");
            res.redirect("/book");
        }

        book.quantity --;
        await borrowerService.purchaseBook({ userId, bookId }, next);
        req.flash("succes",`Successfully purchased book ${book.book_name}`);
        res.redirect("/book");
    },
    returnBook: async (req, res, next) => {
        ...
        if (purchasedBook && !purchasedBook?.active) {
            ...
            req.flash("error","Book is already returned.");
        ...
        if (!book) {
            // then show error
            req.flash("succes", "Book id does not exist or maybe deleted.");
